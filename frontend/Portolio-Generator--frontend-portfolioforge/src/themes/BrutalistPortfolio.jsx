...
export default function BrutalistPortfolio({ data }) {
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans selection:bg-black selection:text-white pb-24">
      {/* HEADER */}
      <header className="bg-black text-white p-6 border-b-8 border-black flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div className="flex items-center gap-4">
          {(data.profileImage || data.personalInfo?.profileImage) && (
            <img
              src={data.profileImage || data.personalInfo?.profileImage}
              alt={data.name}
              className="w-16 h-16 rounded-full object-cover border-4 border-white brutalist-shadow flex-shrink-0"
            />
          )}
        ...
        <div className="flex gap-4">
          {data.email && <a href={`mailto:${data.email}`} className="hover:underline font-black uppercase text-xl text-yellow-300">Email</a>}
      ...
      <main className="px-6 mt-12 max-w-screen-2xl mx-auto space-y-24">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {(data.education && data.education.length > 0) && (
            <section className="border-4 border-black p-8 bg-white">
              <div className="text-xs uppercase tracking-widest bg-black text-white px-3 py-1 mb-6 inline-block font-bold">
                EDUCATION
              </div>
              <div className="space-y-6">
                {data.education.map((edu, index) => (
                  <div key={edu.id || index}>
                    ...
                    <div className="text-lg font-bold text-gray-700 uppercase mt-1">{edu.university} ({edu.startYear}–{edu.endYear})</div>
                  </div>
                ))}
              </div>
            </section>
            ...
            <section className="border-4 border-black p-8 bg-white">
              ...
              <div className="space-y-4">
                {data.certifications.map((cert, index) => (
                  <div key={cert.id || index} className="font-bold text-lg uppercase">
                    🏆 {cert.name} <span class="text-sm font-normal">({cert.issuer})</span>
                  </div>
                ))}
              </div>
            </section>
        ...
        {(data.achievements && data.achievements.length > 0) && (
          <section className="border-4 border-black p-8 bg-yellow-300">
            <div className="text-xs uppercase tracking-widest bg-black text-white px-3 py-1 mb-6 inline-block font-bold">
              ACHIEVEMENTS & AWARDS
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {data.achievements.map((ach, index) => (
                <div key={ach.id || index} className="border-2 border-black p-4 bg-white font-bold">
                  <h4 className="text-xl font-black uppercase mb-2">⭐ {ach.title}</h4>
                  <p className="text-sm uppercase text-gray-700">{ach.description}</p>
                </div>
              ))}
            </div>
          </section>
