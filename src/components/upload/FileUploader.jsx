...
export default function FileUploader({ onFileSelect, accept = '.txt', className = '' }) {
  ...
  const validate = (f) => {
    ...
    if (accept === '.txt' && !f.name.endsWith('.txt')) {
      return `Unsupported file format. Please upload a ${accept} file.`;
    }
    if (f.size === 0) return 'The file appears to be empty.';
    if (f.size > 5 * 1024 * 1024) return 'File is too large. Maximum 5MB.';
    return '';
  };
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        <p className="mt-4 text-xs text-gray-400 dark:text-gray-500">Supported format: .txt · Max 5MB</p>
      </label>
