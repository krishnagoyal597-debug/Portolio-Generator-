...
export default function GenerateStep() {
  ...
  const { setGeneratedJSON } = usePortfolio();
  ...
  useEffect(() => {
    ...
    const runStage = () => {
      if (currentIdx >= STAGES.length) {
        setDone(true);
        setGeneratedJSON(mockGeneratedJSON);
        return;
      }
      setTimeout(() => {
        currentIdx++;
        ...
        <div className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
          ...
          {jsonOpen && (
            <div className="p-4 bg-gray-900 text-gray-300 text-xs font-mono overflow-auto max-h-60 relative">
              <button 
                onClick={(e) => { e.stopPropagation(); navigator.clipboard.writeText(JSON.stringify(mockGeneratedJSON, null, 2)); setCopied(true); setTimeout(() => setCopied(false), 2000); }}
