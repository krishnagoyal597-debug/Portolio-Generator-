...
export default function UploadStep() {
  ...
  const { setResumeId } = usePortfolio();
  ...
  const validateFile = (f) => {
    ...
    if (!f.name.endsWith('.txt')) return 'Unsupported format. Please upload a .txt file.';
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400">Upload your plain-text resume and we'll analyze it automatically.</p>
      </div>
      ...
      <div
        ...
        `}
      >
        <input ref={fileInputRef} type="file" accept=".txt" className="hidden" onChange={(e) => handleFile(e.target.files[0])} />
        {!file ? (
          <>
            ...
            <p className="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Drag & drop your .txt resume here</p>
            ...
            <p className="text-xs text-gray-400 mt-3">.txt files only · Max 5MB</p>
          </>
        ) : (
      ...
      <div className="p-4 rounded-xl bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800 mb-8">
        <p className="text-sm font-semibold text-indigo-800 dark:text-indigo-200 mb-2">💡 Tips for best results</p>
        <ul className="text-xs text-indigo-700 dark:text-indigo-300 space-y-1 list-disc list-inside">
          <li>Save your resume as plain text (.txt format)</li>
          <li>Include sections: Education, Skills, Experience, Projects</li>
          <li>Use clear section headings (e.g., "EDUCATION", "SKILLS")</li>
          <li>Include contact info at the top</li>
        </ul>
      </div>
      ...
      {!uploaded && <p className="text-center text-xs text-gray-400 mt-3">Upload a .txt file to continue</p>}
    </div>
  );
}
